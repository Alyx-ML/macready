...
const AXIS_STYLE = { fontSize: 11, fontFamily: '"Aeonik", system-ui' };
    ...
    <g style={{ pointerEvents: "none" }}>
      ...
      {lines.map((line, i) => (
        <text key={i} x={x + 8 + padX} y={y - h / 2 + padY + (i + 1) * lineH - 4}
          fill="var(--dac-text-primary, #111)" fontSize={11} fontFamily='"Aeonik", system-ui'>
          {line}
        </text>
      ))}
    </g>
...
function CalendarHeatmap({
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{ maxHeight: 180 }}
      ...
      {months.map((m, i) => (
        <text key={i} x={30 + m.week * (cellSize + gap)} y={10} fill={axisColor} fontSize={9} fontFamily='"Aeonik", system-ui'>
          {m.label}
        </text>
      ))}
      {weeks.map((cell, i) => {
...
function HeatmapChart({
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{ maxHeight: 240 }}
      onMouseLeave={() => setHover(null)}>
      {xLabels.map((x, i) => (
        <text key={`x-${i}`} x={leftPad + i * cellW + cellW / 2} y={14} fill={axisColor} fontSize={9} fontFamily='"Aeonik", system-ui' textAnchor="middle">
          {formatAxisTick(x)}
        </text>
      ))}
      {yLabels.map((y, j) => (
        <text key={`y-${j}`} x={leftPad - 4} y={topPad + j * cellH + cellH / 2 + 3} fill={axisColor} fontSize={9} fontFamily='"Aeonik", system-ui' textAnchor="end">
          {String(y).slice(0, 8)}
        </text>
      ))}
      {xLabels.map((x, i) =>
      ...
      {items.map((item, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={leftPad - 6} y={y + 3} fill={axisColor} fontSize={10} fontFamily='"Aeonik", system-ui' textAnchor="end">
          ...
          <FunnelChart>
            ...
            >
              <LabelList
                position="center"
                fill="#fff"
                style={{ fontSize: 11, fontFamily: '"Aeonik", system-ui', fontWeight: 500 }}
